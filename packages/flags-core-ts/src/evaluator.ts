...
import { FlagConfig, Context, EvaluationResult, Rule } from './types';
import { evaluateConditions } from './conditions';
import { computeRolloutBucket } from './rollout';
...
export class Evaluator {
  ...
  private evaluateRule(
    ...
    context: Context,
    flagKey: string
  ): { matched: boolean; reason: 'rule_match' | 'rollout' | 'rollout_excluded'; bucket?: number } {
    ...
    if (rule.percentage !== undefined && rule.percentage < 100) {
      ...
      const bucket = computeRolloutBucket(context.userId, flagKey);
